...
public class AdminRepoImp {
	...
	public boolean isAdminLogin(AdminVO adminvo) {
		...
		if(adminvo.getPassword().equals(admin.getPassword()) &&	adminvo.getAdminname().equals(admin.getAdminname())) {
			return true;
		}
		else{
			return false;
	...
	public AccountDetails getAccountDetails(String adminName,String password, long accountNumber, long aid){
		...
		if(accountDetails.getAccountnumber()==0) {return new AccountDetails("Account number is worng"); }else {
		return accountDetails;}
		}
		catch(Exception e) {return new AccountDetails("Account number is worng");}
		}
		else {
	...
	public String registerUser(String adminName,String password, long aid,UserVO uservo) {
		...
		}catch (Exception e) {return "user already registered" ;}
		}else
		{
	...
	public String createAccount(String adminName,String password, long uid, AccountDetailsVO accountDetailsvo, long aid) {
		if(isAdminLogin(aid,adminName,password)) {
		try{User user=userRepoImp.getById(uid);
		...
		accountDetails.setBaid(accountDetailsvo.getBaid());
		...
		}else {return "Enter valid user id";}
		
		 
		 return "Bank account successfuly created";
		}catch(Exception e) {return "Account number or cif number is not available";}
		}
		else {
	...
	public String deleteAccountDetails(String adminName,String password, long accountNumber, long aid) {
		...
		}catch(Exception e) {return "bank account number is not valid"; }
		}
		return "Enter valid adminName and password";
	}catch(Exception e) {return "Enter valid Admin id";}
	}
